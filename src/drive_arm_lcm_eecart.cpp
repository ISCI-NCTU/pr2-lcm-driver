...
public:
  ...
  void startTrajectory(pr2_controllers_msgs::JointTrajectoryGoal goalL, pr2_controllers_msgs::JointTrajectoryGoal goalR)
  {
    ...
    goalL.trajectory.header.stamp = ros::Time::now() + ros::Duration(81.0);
    goalR.trajectory.header.stamp = ros::Time::now() + ros::Duration(81.0);
...
//our goal variable
  ...
  for(int ind=1; ind<nstates; ind++){
    ...
    if(isleft){
        ...
        printf("%dL %.3f %.3f %.3f %.3f %.3f %.3f %.3f  %f\n", 
          ind,
        ...
        printf("%dR %.3f %.3f %.3f %.3f %.3f %.3f %.3f  %f\n", 
          ind,
